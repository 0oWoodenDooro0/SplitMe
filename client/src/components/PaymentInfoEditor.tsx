...
import { CreditCard, Upload, Trash2, X, Check, Building2, Image as ImageIcon } from 'lucide-react';
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/70">
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">提供收款銀行帳號或 QR Code</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto">
            ...
            {customQrUrl ? (
              <div className="flex items-center gap-3 p-3 bg-slate-50 border border-slate-200 rounded-2xl">
                ...
                <div className="flex-1 space-y-1.5">
                  <span className="text-xs font-medium text-emerald-700 block">已上傳收款 QR Code</span>
                  <button
                    type="button"
                    onClick={handleRemoveQr}
                    className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-semibold text-rose-600 hover:text-rose-700 hover:bg-rose-50 rounded-lg transition-colors border border-rose-200 cursor-pointer"
                  >
          ...
          <div className="pt-3 flex items-center justify-end gap-2">
            ...
            <button
              type="submit"
              className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-700 active:scale-95 rounded-xl shadow-xs transition-all cursor-pointer"
            >
